...
| `--json` | | `{output, outputs: [{output, seed, length_s}], prompt, duration, loop, sample_rate, channels, steps, cfg, device, seconds, peak_vram_gb}` or `{"error": ...}` |
...
- A take can still contain two events: use `--count` and pick, or ship several and randomise playback.
