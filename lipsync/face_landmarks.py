...
Detect face landmarks on a rendered image with MediaPipe Face Landmarker (478 points, FaceMesh topology).
    ...
    lipsync/.venv/bin/python lipsync/face_landmarks.py face.png landmarks.json [--debug debug.png [--draw-on img.png]]

Writes {"width", "height", "points": [[x_px, y_px, z], ...]} (478 points, pixels from the top-left) or exits 2 when
no face is found. Only the whole image is searched: on crops of stylised heads, the detector also finds faces in brow
wrinkles and nostrils, and a mouth cut there passes every later check. Runs in its own venv (mediapipe does not
...
draw_on = argv[argv.index('--draw-on') + 1] if '--draw-on' in argv else src  # the debug image's background
